...
$sqlEntrada_Bodega = "SELECT eb.idbodega_entrada, 
                             ...
                             eb.numentrada_bodega, eb.lunes, eb.martes, eb.miercoles, 
                             eb.jueves, eb.viernes, eb.sabado, eb.domingo, eb.total
                      FROM entrada_bodega AS eb
                      INNER JOIN categorias AS ct ON eb.id_categoria_bodega = ct.id_categoria
                      INNER JOIN productos AS prod ON eb.id_producto_bodega = prod.id_producto
                      INNER JOIN unidad_medida AS um ON eb.id_unidadm_bodega = um.id_medida";

$entrada_bodega_result = $conn->query($sqlEntrada_Bodega);

if (!$entrada_bodega_result) {
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="vistas/css/entradabodega.css">
  <link rel="stylesheet" type="text/css" href="vistas/css/notificacion.css">
...
<body>
  ...
  <!--Texto sencillo-->
  <h1 class="txt">ENTRADA BODEGA</h1>
  ...
  <div class="row justify-content-end">
    <div class="col-auto">
      <a href="vistas/fpdf/rEntradaBodega.php" target="_blank" class="btn btn-success btn-generar-reporte">
        <img src="vistas/iconos/svg/pdf.svg" alt="Generar reporte" class="me-2">
        Generar reporte
      </a>
      <a href="" class="btn btn-success btn-agregar-entrada me-2 " data-bs-toggle="modal" data-bs-target="#MEBodegaN">
    ...
    <div class="row">
      <div class="col text-center">
        ...
        <div class="table-responsive">
          <table class="table table-hover table-striped mt-4" id="tableentrada">
            ...
            <tbody>
              <?php while ($row_entradabodega = $entrada_bodega_result->fetch_assoc()) { ?>
                <tr>
                  <td><?= $row_entradabodega['idbodega_entrada']; ?></td>
                  <td><?= $row_entradabodega['categoria']; ?></td>
                  <td><?= $row_entradabodega['producto']; ?></td>
                  <td><?= $row_entradabodega['unidad_medida']; ?></td>
                  <td><?= $row_entradabodega['numentrada_bodega']; ?></td>
                  <td><?= $row_entradabodega['lunes']; ?></td>
                  <td><?= $row_entradabodega['martes']; ?></td>
                  <td><?= $row_entradabodega['miercoles']; ?></td>
                  <td><?= $row_entradabodega['jueves']; ?></td>
                  <td><?= $row_entradabodega['viernes']; ?></td>
                  <td><?= $row_entradabodega['sabado']; ?></td>
                  <td><?= $row_entradabodega['domingo']; ?></td>
                  <td><?= $row_entradabodega['total']; ?></td>
                  <td>
                    <!-- Botón para Editar Registro -->
                    <a href="#" class="btn btn-sm btn-warning btn-action" data-bs-toggle="modal" data-bs-target="#editaEBodega" data-bs-id="<?= $row_entradabodega['idbodega_entrada']; ?>">
                    ...
                    <a href="entradabodega.php?id=<?= $row_entradabodega['idbodega_entrada']; ?>" onclick="advertencia(event)" class="btn btn-sm btn-danger btn-action"">
  ...
  <?php include 'modalagentbodegaNUEVO.php'; ?>
  ...
  <script>
    let editaModal = document.getElementById('modaleditentbodega')
    ...
    editaModal.addEventListener('shown.bs.modal', event => {
      ...
      let inputProducto = editaModal.querySelector('.modal-body #producto')
      let inputMedida = editaModal.querySelector('.modal-body #medida')
      let inputEntradaB = editaModal.querySelector('.modal-body #entradab')
      let inputLunes = editaModal.querySelector('.modal-body #lunes')
      let inputMartes = editaModal.querySelector('.modal-body #martes')
      let inputMiercoles = editaModal.querySelector('.modal-body #miercoles')
      let inputJueves = editaModal.querySelector('.modal-body #jueves')
      let inputViernes = editaModal.querySelector('.modal-body #viernes')
      let inputSabado = editaModal.querySelector('.modal-body #sabado')
      let inputDomingo = editaModal.querySelector('.modal-body #domingo')

      /* Hacemos una consulta SQL con AJAX y traemos la información de los campos */
      let url = "getEntradaBodega.php"
      /* Para pasar los datos */
      let formData = new FormData()
      formData.append('idbodega_entrada', id)
      ...
      fetch(url, {
        ...
        .then(data => {
          ...
          inputCategoria.value = data.id_categoria_bodega
          inputProducto.value = data.id_producto_bodega
          inputMedida.value = data.id_unidadm_bodega
          inputEntradaB.value = data.numentrada_bodega
