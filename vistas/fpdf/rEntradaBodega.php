...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(20);  // mover a la derecha
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(85, 10, utf8_decode("Módulo: Entrada-Bodega"), 0, 0, '', 0);
        $this->Ln(10);
        ...
        $this->Cell(22, 10, utf8_decode('ENTRADA'), 1, 0, 'C', 1);
...
$consulta = "SELECT eb.idbodega_entrada, c.nombre_cat, p.nombre_prod, um.uni_med, 
                    eb.numentrada_bodega, eb.lunes, eb.martes, eb.miercoles, eb.jueves, 
                    eb.viernes, eb.sabado, eb.domingo, eb.total
             FROM entrada_bodega eb
             JOIN categorias c ON eb.id_categoria_bodega = c.id_categoria
             JOIN productos p ON eb.id_producto_bodega = p.id_producto
             JOIN unidad_medida um ON eb.id_unidadm_bodega = um.id_medida";
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $pdf->Cell(15, 10, $row['idbodega_entrada'], 1, 0, 'C', 0);
    ...
    $pdf->Cell(22, 10, $row['numentrada_bodega'], 1, 0, 'C', 0);
...
$pdf->Output('ReporteEntradaBodega.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
