...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(20);  // mover a la derecha
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(85, 10, utf8_decode("Módulo: Entrada-Cocina"), 0, 0, '', 0);
        $this->Ln(10);
...
$consulta = "SELECT ec.idcocina_entrada, c.nombre_cat, p.nombre_prod, um.uni_med, 
                    ec.inicial_cocina, ec.lunes, ec.martes, ec.miercoles, ec.jueves, 
                    ec.viernes, ec.sabado, ec.domingo, ec.total
             FROM entrada_cocina ec
             JOIN categorias c ON ec.id_categoria_cocina = c.id_categoria
             JOIN productos p ON ec.id_producto_cocina = p.id_producto
             JOIN unidad_medida um ON ec.id_unidadm_cocina = um.id_medida";
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $pdf->Cell(15, 10, $row['idcocina_entrada'], 1, 0, 'C', 0);
    ...
    $pdf->Cell(22, 10, $row['inicial_cocina'], 1, 0, 'C', 0);
...
$pdf->Output('ReporteEntradaCocina.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
