...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(20);  // mover a la derecha
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(85, 10, utf8_decode("Módulo: Salida-Cocina"), 0, 0, '', 0);
        $this->Ln(10);
...
$consulta = "SELECT sc.idcocina_salida, c.nombre_cat, p.nombre_prod, um.uni_med, 
                    sc.inicial_scocina, sc.lunes, sc.martes, sc.miercoles, sc.jueves, 
                    sc.viernes, sc.sabado, sc.domingo, sc.actual
             FROM salida_cocina sc
             JOIN categorias c ON sc.id_categoria_scocina = c.id_categoria
             JOIN productos p ON sc.id_producto_scocina = p.id_producto
             JOIN unidad_medida um ON sc.id_unidadm_scocina = um.id_medida";
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $pdf->Cell(15, 10, $row['idcocina_salida'], 1, 0, 'C', 0);
    ...
    $pdf->Cell(22, 10, $row['inicial_scocina'], 1, 0, 'C', 0);
...
$pdf->Output('ReporteSalidaCocina.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
